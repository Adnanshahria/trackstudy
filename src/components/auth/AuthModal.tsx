    ...
    return (
        <Modal isOpen={props.isOpen} onClose={props.onClose} title={props.modalMode === 'login' ? 'Sign In' : props.modalMode === 'create' ? 'Create Account' : props.modalMode === 'change' ? 'Change Password' : 'Reset Password'}>
            <div className="flex flex-col gap-4 md:gap-6">
                <div className="flex p-0.5 md:p-1 bg-slate-200/50 dark:bg-black rounded-full relative backdrop-blur-sm">
                    <button onClick={() => { props.setModalMode('login'); props.resetModalState(); }} className={`flex-1 py-1.5 md:py-2 text-[10px] md:text-xs font-bold rounded-full transition-all duration-300 z-10 ${props.modalMode === 'login' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-white'}`}>Sign In</button>
                    <button onClick={() => { props.setModalMode('create'); props.resetModalState(); }} className={`flex-1 py-1.5 md:py-2 text-[10px] md:text-xs font-bold rounded-full transition-all duration-300 z-10 relative overflow-hidden ${props.modalMode === 'create' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-white'}`}>
                ...
                {props.modalError && (props.modalError.toLowerCase().includes('not found') || props.modalError.toLowerCase().includes('no user')) && (
                    <div className="text-[9px] md:text-[10px] text-center text-slate-600 dark:text-slate-400 bg-blue-50 dark:bg-blue-900/20 p-2 md:p-3 rounded-lg border border-blue-200 dark:border-blue-700/50 animate-fade-in">
                        <p className="mb-1.5 md:mb-2">This User ID doesn't exist yet.</p>
                ...
                {isDomainError && (
                    <div className="text-[9px] md:text-[10px] p-1.5 md:p-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 rounded-lg text-amber-800 dark:text-amber-200 animate-fade-in">
                        ...
                        <code className="bg-amber-100 dark:bg-black/30 px-1 py-0.5 rounded mt-1 block w-fit text-[8px] md:text-[9px]">{window.location.hostname}</code>
                    </div>
                )}
            </div>
        </Modal>
