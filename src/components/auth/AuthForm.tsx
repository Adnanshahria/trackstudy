        ...
        <div className="flex flex-col gap-3 md:gap-5">
            ...
            <div className="flex flex-col gap-2 md:gap-3 mt-2 md:mt-4">
                <div className="relative group p-0.5 md:p-1 rounded-xl md:rounded-2xl bg-gradient-to-r from-blue-500/20 via-blue-400/10 to-blue-500/20 backdrop-blur-lg border border-blue-300/30 dark:border-blue-400/20 shadow-lg shadow-blue-500/10 dark:shadow-blue-600/20">
                    <Button onClick={handleUserAction} disabled={!tempUserId.trim() || isChecking || (modalMode === 'reset' && !!recoveredPassword)} className="w-full py-3 md:py-4 text-xs md:text-sm rounded-lg md:rounded-xl">{isChecking ? 'Processing...' : (modalMode === 'login' ? 'Sign In' : modalMode === 'create' ? 'Create Account' : modalMode === 'change' ? 'Change Password' : 'Recover Password')}</Button>
                </div>
                {modalMode === 'login' && (
