        ...
        <Modal isOpen={true} onClose={onClose} title={`Add Chapter to Paper ${paper}`}>
            <div className="flex flex-col gap-3 md:gap-4">
                <div className="flex flex-col gap-1">
                    <label className="text-[10px] md:text-xs text-slate-500">Chapter Name</label>
                    <input
                        type="text"
                        value={name}
                        onChange={(e) => setName(e.target.value)}
                        className="bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-md md:rounded-lg p-1.5 md:p-2 text-xs md:text-sm focus:outline-none focus:border-blue-500 dark:text-white"
