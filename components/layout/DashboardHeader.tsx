...
export const DashboardHeader: React.FC<{ onDev: () => void, status: string, userId: string, userData: UserData, onLogout: () => Promise<void>, onToggleTheme: () => void, theme: any, onGuide: () => void, onAppearance: () => void, onForceSync: () => void, onOpenAdmin?: () => void, onOpenMigration?: () => void }> = ({ onDev, status, userId, userData, onLogout, onToggleTheme, theme, onGuide, onAppearance, onForceSync, onOpenAdmin, onOpenMigration }) => (
    <div className="flex flex-row justify-between items-center mb-6 no-print gap-4">
        ...
        <div className="flex items-center gap-4 shrink-0">
            <SettingsMenu userId={userId} userData={userData} onLogout={onLogout} onToggleTheme={onToggleTheme} theme={theme} onOpenGuide={onGuide} onOpenDevModal={onDev} onOpenAppearance={onAppearance} onForceSync={onForceSync} onOpenAdmin={onOpenAdmin} onOpenMigration={onOpenMigration} />
        </div>
    </div>
);
