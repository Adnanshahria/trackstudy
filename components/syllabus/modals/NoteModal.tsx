...
export const NoteModal = ({ isOpen, onClose, noteKey, text, onSave, setText }: { isOpen: boolean, onClose: () => void, noteKey: string, text: string, onSave: () => void, setText: (s: string) => void }) => {
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Study Note">
            <div className="flex flex-col gap-4">
                <p className="text-xs text-slate-500 dark:text-slate-400">ID: {noteKey}</p>
                <textarea 
