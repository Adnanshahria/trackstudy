...
export const SyllabusModals: React.FC<Props> = ({ modals, setModals, handlers, ui, activeSubject, settings }) => {
    return (
        <>
            {modals.note && <NoteModal isOpen={true} onClose={ui.closeNote} noteKey={modals.note.key} text={modals.note.text} onSave={() => { handlers.onUpdateNote(modals.note.key, modals.note.text); ui.closeNote(); }} setText={(s) => setModals((m: any) => ({...m, note: {...m.note, text: s}}))} settings={settings} activeSubject={activeSubject} />}
