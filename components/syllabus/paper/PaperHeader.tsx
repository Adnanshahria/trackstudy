    ...
    <div className="relative flex items-center justify-between p-4 bg-slate-50/50 dark:bg-white/5 hover:bg-white dark:hover:bg-white/10 transition-colors z-10 print:bg-gray-100 border-b-2 border-slate-200 dark:border-white/10">
        ...
        <div className="flex items-center gap-3 no-print">
            {editMode && onDeletePaper && (
                <button onClick={(e) => { e.stopPropagation(); onDeletePaper(activeSubject, paper); }} className="w-9 h-9 flex items-center justify-center rounded-full transition-all text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10">✕</button>
            )}
